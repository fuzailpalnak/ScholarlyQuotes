...
pub async fn fetch_qotd_by_language(
    ...
    match get_quote_from_cache_or_db(db_conn, redis, language).await {
        Ok(quote) => Ok(quote),
        Err(e) => Err(e),
    }
}
    ...
    match redis_conn.get::<_, String>(key.clone()).await {
        Ok(cached_quote) => match serde_json::from_str::<model::ResponseQuote>(&cached_quote) {
            ...
            Err(e) => {
                ...
                queries::fetch_and_cache_qotd_from_db(db_conn, &mut redis_conn, language).await
            }
        },
        Err(e) => {
...
async fn get_quotes_by_language(
    ...
    let response = queries::fetch_random_quote_by_language(db_conn, language).await?;
